...
export class TipoDenunciaComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.tipoDenunciaService.search({
    ...
    trackId(index: number, item: TipoDenuncia) {
        return item.id;
    }
    registerChangeInTipoDenuncias() {
        this.eventSubscriber = this.eventManager.subscribe('tipoDenunciaListModification', (response) => this.reset());
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.tipoDenuncias.push(data[i]);
        }
    }
