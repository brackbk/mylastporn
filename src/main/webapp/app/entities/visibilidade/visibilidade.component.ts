...
export class VisibilidadeComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.visibilidadeService.search({
    ...
    trackId(index: number, item: Visibilidade) {
        return item.id;
    }
    registerChangeInVisibilidades() {
        this.eventSubscriber = this.eventManager.subscribe('visibilidadeListModification', (response) => this.reset());
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.visibilidades.push(data[i]);
        }
    }
